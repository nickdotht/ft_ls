...
void format_handler(t_dirs **dir, struct stat file_stat) {
  if ((*dir)->format.link < ft_intlen(file_stat.st_nlink))
    (*dir)->format.link = ft_intlen(file_stat.st_nlink);
  ...
  if ((*dir)->format.fileSize < ft_intlen(file_stat.st_size))
    (*dir)->format.fileSize = ft_intlen(file_stat.st_size);
}
